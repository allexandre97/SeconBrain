...
- **Answer note**: A readable generated Markdown response to a user question about the knowledge base, stored under `wiki/answers/` for later review in Obsidian.
- **Durable wiki page**: A source, concept, question, claim, tension, entity, overview, or category page that stores reusable knowledge beyond a single generated answer.
- **Targeted source lookup**: A staged raw-source lookup strategy that starts from source summaries, outlines, inventories, proof maps, headings, and local search before reading broader source sections.
- **Raw-source consultation**: Reading original material in `raw/` to answer a query when existing wiki pages are insufficient, without treating that lookup as ingestion unless explicitly requested.
- **Answer status**: Answer-page frontmatter that records whether a persistent answer note is `answered`, `partially-answered`, or `needs-review`.
